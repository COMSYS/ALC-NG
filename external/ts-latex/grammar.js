...
// has to match order of array in scanner.c
module.exports = grammar({
  ...
  rules: {
    ...
    block_comment: $ =>
      choice(
        seq(
          field('begin', choice('\\iffalse', '\\if0')),
          field('comment', optional(alias($._trivia_raw_else, $.comment))),
          optional($._textual_else_clause),
          field('end', '\\fi'),
        ),
        seq(
          field('begin', choice('\\iftrue', '\\if1')),
          repeat($._root_content),
          optional($._commental_else_clause),
          field('end', '\\fi'),
        ),
        $.general_if,
      ),

    if_content: $ => repeat1($._root_content),
    else_content: $ => repeat1($._root_content),

    _textual_else_clause: $ =>
      seq('\\else', field('else', optional($.else_content))),

    _commental_else_clause: $ =>
      seq(
        '\\else',
        field('comment', optional(alias($._trivia_raw_fi, $.comment))),
      ),

    general_if: $ => seq(choice($._general_ifthenelse, $._general_if_normal)),

    iff_command: $ => prec.right(seq(field('command', '\\iff'))),

    _general_ifthenelse: $ => seq(
      field('begin', "\\ifthenelse"),
      field('if', $.curly_group),
      field('then', $.curly_group),
      field('else', $.curly_group),
    ),

    _general_if_normal: $ => seq(
      field('begin', /\\if[a-z]+/),
      field('then', optional($.if_content)),
      optional($._textual_else_clause),
      field('end', '\\fi')
    ),
    ...
    curly_group_impl: $ =>
      seq('{', repeat(choice($._text_content, $.begin, $.end, '[', ']', ',', '=')), '}'),

    curly_group_author_list: $ =>
      seq(
        '{',
        sepBy(
          alias(repeat1(choice($._text_content, '[', ']')), $.author),
    ...
    brack_group_text: $ => seq('[', repeat(choice(field('text', $.text), $._math_content, '(', ')', $.curly_group)), ']'),
    ...
    ...specialEnvironment({
      ...
      name: 'verbatim',
      content: $ =>
        field('verbatim', alias($._trivia_raw_env_verbatim, $.verbatim)),
      options: undefined,
    }),
    ...
    new_if: $ =>
      seq(
        '\\newif',
        field("name", $.new_if_name),
      ),

    new_if_name: $ => /\\if[a-zA-Z@]+/,

    new_if_assign_true: $ => /\\[a-zA-Z@]+true/,
    new_if_assign_false: $ => /\\[a-zA-Z@]+false/,
    ...
    latex_include: $ =>
      seq(
        ...
        field('path', choice($.curly_group_path, $.path)),
      ),
    ...
    _new_command_definition: $ =>
      seq(
        ...
        field('implementation', $.curly_group_impl),
      ),
    ...
    long_command_definition: $ =>
      seq(
        '\\long',
        choice($.new_command_definition, $.old_command_definition)
      ),
    ...
    _environment_definition: $ =>
      seq(
        ...
        field('name', $.curly_group_text),
        optional($._arguments_definition),
