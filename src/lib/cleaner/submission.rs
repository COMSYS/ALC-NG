...
impl Submission {
    ...
    pub fn new(
        ...
        debug!("Target path: {:?}", target_path.as_ref());
        debug!("LaTeX command: {}", latex_cmd);
        debug!("LaTeX parameters: {:?}", latex_parameters);
        ...
        debug!("Created temporary cache directory: {:?}", cache_dir.path());
        ...
        {
            Some(Ok(zzrm)) => {
                debug!("Successfully parsed 00Readme file");
                Some(zzrm)
            }
            None => {
                debug!("00Readme parsing skipped (no_zzrm is enabled)");
                None
            }
            Some(Err(e)) => {
                info!(
    ...
    fn compile(&mut self) {
        ...
        debug!("Starting LaTeX compilation phase");
        let main_files = self.get_mains();
        debug!("Found {} main LaTeX file(s) to compile", main_files.len());
        for main_file in &main_files {
            debug!("Main file: {:?}", main_file.relative());
        }
        ...
        if commands.is_empty() {
            ...
            debug!("Executing {} LaTeX compilation command(s)", commands.len());
        }
        ...
        self.latex_output = commands
            ...
            .map(|(main_file, mut cmd)| {
                debug!("Compiling main file: {:?}", main_file.relative());
                let result = cmd.output();
                match &result {
                    Ok(output) => {
                        if output.status.success() {
                            debug!("Compilation successful for {:?}", main_file.relative());
                        } else {
                            warn!(
            ...
            .collect();
        debug!("LaTeX compilation phase completed");
    }
    ...
    pub fn determine_latexmk_bib_references(&mut self) {
        ...
        debug!("Detecting BibTeX references from latexmk output");

        self.latexmk_referenced_bibs = self
            .latex_output // get the map of main files to compiler outputs
            .iter() // iterate over (&SourceFile, Result<Output>) pairs
            .map(|(_, o)| o.as_ref()) // keep only successful outputs
            .filter_map(Result::ok_with_warning)
            .map(|o| String::from_utf8_lossy(&o.stdout[..])) // decode stdout as UTF-8 lossily
            .map(|content| find_referenced_bibs(content.as_ref(), &self.cache_path)) // find bib files in the output
            .flatten() // flatten nested iterators into a single iterator of SourceFile
    ...
    fn process_recorder_files(&mut self) -> Result<()> {
        ...
        debug!("Processing .fls recorder files");
        let full_cache_path = self.cache_path.path().canonicalize()?;
        debug!("Cache path: {:?}", full_cache_path);
        ...
        debug!("Copy and clean phase completed");
        Ok(())
    }
    ...
    fn copy_and_clean(&mut self) -> Result<()> {
        ...
        debug!("Starting copy and clean phase");
        debug!(
        ...
        let all_used: HashSet<_> = self
            ...
            .collect();
        debug!("Total files marked as used: {}", all_used.len());
        ...
        for parsed in &self.initial_file_list {
            ...
            let _ = match (
                ...
                (_, Some(FileUsage::TopLevel), _) | (true, _, _) => {
                    ...
                    parsed.copy_cleaned_to(
                        &target,
                        &mut self.grammar_errors,
                        &mut self.deletion_stats,
                    )?;
        ...
        if !self.cleaner_config.skip_watermark {
            for file in &self.possible_main_files {
                ...
                if let Ok(mut handle) = handle {
                    handle.write(watermark.as_bytes())?;
                }

                debug!("Added watermark to {:?}", file.relative());
            }
        }
    ...
    pub fn pack_as_tar(&self) -> Result<()> {
        ...
        let output_file: Box<dyn Write> = match &self.cleaner_config.tar {
            ...
            None => {
                debug!("Tar packaging skipped (no tar option configured)");
                return Ok(());
            }
        };
        ...
        debug!("Added target directory to tar archive");
        // Write termination section of archive
        tar.finish()?;

        debug!("Tar packaging completed successfully");
        Ok(())
    }

    fn latexmk_clean(&self) -> Result<()> {
        let mut cmd = Command::new("latexmk");
        cmd.args(["-cd", "-f", "-pdf", "-interaction=nonstop", "-synctex=1"]);
        cmd.arg("-C");
    ...
    pub fn compare(&self) -> Result<HashMap<String, Option<Vec<bool>>>> {
        use walkdir::WalkDir;
        ...
        Ok(self
            ...
            .map(|f| {
                ...
                if !original.exists() || !new_version.exists() {
                    return (f.relative().display().to_string(), None);
                }
                ...
                (f.relative().display().to_string(), r.ok_with_warning())
            })
            .collect())
    }
    ...
    pub fn run(&mut self) -> Result<()> {
        ...
        if self.cleaner_config.strip_exif {
            ...
            debug!("EXIF stripping skipped (not configured)");
        }

        debug!("Full cleaning pipeline completed successfully");
        Ok(())
    }
